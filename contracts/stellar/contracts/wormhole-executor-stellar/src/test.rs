...
/// Owner emitter (governance actions) used in unit tests.
const OWNER_EMITTER_CHAIN: u32 = 1;
const OWNER_EMITTER_ADDRESS: [u8; 32] = [0x42u8; 32];
/// Guardian-set-upgrade emitter — distinct chain *and* address from the owner.
const GS_UPGRADE_EMITTER_CHAIN: u32 = 2;
const GS_UPGRADE_EMITTER_ADDRESS: [u8; 32] = [0x77u8; 32];
...
fn setup_contract(
    ...
    let owner_emitter_address = BytesN::from_array(env, &OWNER_EMITTER_ADDRESS);
    let gs_upgrade_emitter_address = BytesN::from_array(env, &GS_UPGRADE_EMITTER_ADDRESS);
...
fn test_verify_vaa_success_quorum() {
    ...
    let signers: alloc::vec::Vec<(u8, [u8; 32])> =
        (0..3).map(|i| (i as u8, secrets[i as usize])).collect();
...
fn test_verify_vaa_quorum_with_19_guardians() {
    ...
    let signers: alloc::vec::Vec<(u8, [u8; 32])> =
        (0..13).map(|i| (i as u8, secrets[i as usize])).collect();
...
fn test_verify_vaa_duplicate_guardian_index() {
    ...
    env.as_contract(&contract_id, || {
        ...
        assert_eq!(
            result.err(),
            Some(ContractError::DuplicateGuardianSignature)
        );
    });
}
...
#[test]
fn test_verify_vaa_invalid_recovery_id() {
    let env = Env::default();
    let (_, contract_id, secrets) = setup_contract(&env, 1, 0);
    ...
    let body_hash = keccak256(&body);
    let double_hash = keccak256(&body_hash);
    let (sig, _rec) = sign_hash(&secrets[0], &double_hash);

    // Craft a VAA whose recovery_id byte is 0xFF (out of the valid [0, 3]
    // range). The host would panic on secp256k1_recover; the explicit
    // range check should turn this into a clean contract error.
    let mut vaa_raw = alloc::vec::Vec::new();
    vaa_raw.push(1u8);
    vaa_raw.extend_from_slice(&0u32.to_be_bytes());
    vaa_raw.push(1u8);
    ...
    env.as_contract(&contract_id, || {
        let result = verify_vaa(&env, &vaa);
        assert_eq!(result.err(), Some(ContractError::InvalidRecoveryId));
    });
}
...
#[test]
fn test_guardian_set_upgrade_wrong_emitter_chain() {
    let env = Env::default();
    let (client, _, secrets) = setup_contract(&env, 1, 0);
    ...
    // emitter_chain = 99 — matches neither the owner nor the
    ...
    assert_eq!(
        ...
        ContractError::InvalidEmitterChain
    );
}

#[test]
fn test_guardian_set_upgrade_wrong_emitter_address() {
    let env = Env::default();
    let (client, _, secrets) = setup_contract(&env, 1, 0);
    ...
    let upgrade_payload = build_guardian_set_upgrade_payload(0, 1, &[new_addr]);
    // Use 0xFF.., which matches neither the owner emitter address nor the
    // guardian-set-upgrade emitter address.
    let body = build_body(
        1000,
        0,
        GS_UPGRADE_EMITTER_CHAIN as u16,
    ...
    assert_eq!(
        result.err().unwrap().unwrap(),
        ContractError::InvalidEmitterAddress
    );
}

/// A guardian-signed VAA from the *owner* (governance) emitter must not be
/// accepted as a guardian set upgrade. This is the headline test for the
/// emitter split: even if the payload is a well-formed Core/action=2 message
/// signed by the current guardian set, the wrong emitter has to be rejected.
/// The owner emitter has a different chain ID *and* address than the
/// guardian-set-upgrade emitter, so the chain check rejects this first.
#[test]
fn test_guardian_set_upgrade_rejects_owner_emitter() {
    let env = Env::default();
    let (client, _, secrets) = setup_contract(&env, 1, 0);
...
/// As above, but uses the owner emitter *address* on the guardian-set-upgrade
/// chain — exercises the address check path of the emitter split.
#[test]
fn test_guardian_set_upgrade_rejects_owner_address_on_upgrade_chain() {
    let env = Env::default();
    let (client, _, secrets) = setup_contract(&env, 1, 0);
...
#[test]
fn test_constructor_stores_separate_emitters() {
    let env = Env::default();
    let (_, contract_id, _) = setup_contract(&env, 1, 0);

    env.as_contract(&contract_id, || {
        assert_eq!(
            guardian::get_owner_emitter_chain(&env),
            Ok(OWNER_EMITTER_CHAIN)
        );
        assert_eq!(
            guardian::get_owner_emitter_address(&env)
                .unwrap()
                .to_array(),
            OWNER_EMITTER_ADDRESS
        );
        assert_eq!(
            guardian::get_gs_upgrade_emitter_chain(&env),
            Ok(GS_UPGRADE_EMITTER_CHAIN)
        );
        assert_eq!(
            guardian::get_gs_upgrade_emitter_address(&env)
                .unwrap()
                .to_array(),
            GS_UPGRADE_EMITTER_ADDRESS
        );
        // Sanity: the two emitters are configured with distinct values.
        assert_ne!(OWNER_EMITTER_CHAIN, GS_UPGRADE_EMITTER_CHAIN);
