...
pub fn parse_vaa(env: &Env, data: &Bytes) -> Result<Vaa, ContractError> {
    ...
    for i in 0..num_signatures {
        ...
        for (j, slot) in sig_bytes.iter_mut().enumerate() {
            *slot = get_byte(data, (sig_offset + 1 + j) as u32)?;
        }
    ...
    for (j, slot) in emitter_addr.iter_mut().enumerate() {
        *slot = get_byte(data, (body_offset + 10 + j) as u32)?;
    }
...
pub fn verify_vaa(env: &Env, vaa: &Vaa) -> Result<(), ContractError> {
    ...
    let digest = env
        .crypto()
        .keccak256(&Bytes::from_slice(env, &body_hash.to_array()));
    ...
    for i in 0..num_sigs {
        ...
        // Soroban's secp256k1_recover requires recovery_id in [0, 3];
        // otherwise the host panics. Reject malformed VAAs explicitly.
        if sig.recovery_id > 3 {
            return Err(ContractError::InvalidRecoveryId);
        }
...
mod tests {
    ...
    fn test_parse_vaa_governance_payload() {
        ...
        assert!(!vaa.body.payload.is_empty());
    }
    ...
    fn test_parse_vaa_13_signatures() {
        ...
        let body = build_body_vec(1700000000, 12345, 1, &emitter_address, 9999, 32, &payload);
