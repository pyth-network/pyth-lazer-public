...
fn setup(env: &Env) -> (PythLazerContractClient<'_>, Address) {
...
#[test]
fn test_verify_update_invalid_recovery_id() {
    let env = Env::default();
    let (client, executor) = setup(&env);
    ...
    // Corrupt the recovery_id byte (offset 68) to a value outside [0, 3].
    // Without the range check, Soroban's secp256k1_recover host call panics.
    let mut update_raw = test_lazer_update_bytes(&env).to_alloc_vec();
    update_raw[68] = 0xFF;
    let update = Bytes::from_slice(&env, &update_raw);

    let result = client.try_verify_update(&update);
    assert_eq!(result.err().unwrap(), Ok(ContractError::InvalidRecoveryId));
}
