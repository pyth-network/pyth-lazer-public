...
pub fn verify_update(env: &Env, data: &Bytes) -> Result<Bytes, ContractError> {
    ...
    // Parse recovery_id (1 byte at offset 68). Soroban's secp256k1_recover
    // requires recovery_id in [0, 3]; otherwise the host panics.
    ...
    let uncompressed_pubkey = env
        .crypto()
        .secp256k1_recover(&hash, &signature, recovery_id);
