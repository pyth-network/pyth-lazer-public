...
Storage (persistent):
  ...
  - guardian_set_upgrade_emitter_chain_id: u16        // authorized guardian-set-upgrade emitter chain
  - guardian_set_upgrade_emitter_address: BytesN<32>  // authorized guardian-set-upgrade emitter address
...
- `initialize(env, wormhole_chain_id, owner_emitter_chain_id, owner_emitter_address, guardian_set_upgrade_emitter_chain_id, guardian_set_upgrade_emitter_address, initial_guardian_set, guardian_set_index)` - One-time setup. Two emitters are stored: the *owner* emitter authorizes PTGM governance actions executed via `execute_governance_action`, and the *guardian-set-upgrade* emitter (the Wormhole core bridge governance emitter) authorizes guardian set upgrades.
